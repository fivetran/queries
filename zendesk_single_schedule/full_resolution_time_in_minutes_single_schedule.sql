...
ticket_full_solved_time as (
  select ticket.id as ticket_id,
         ...
         greatest(0, round(timestamp_diff(max(ticket_full_solved_at.solved_at), ticket.created_at, second)/60, 0)) as raw_delta_in_minutes
  from zendesk.ticket
  left join ticket_full_solved_at on ticket.id = ticket_full_solved_at.ticket_id
...
select ticket_full_solved_time.ticket_id,
       business_minutes.full_resolution_time_in_business_minutes,
       sum(ticket_full_solved_time.raw_delta_in_minutes) as full_resolution_time_in_calendar_minutes
from ticket_full_solved_time
left join business_minutes
  on business_minutes.ticket_id = ticket_full_solved_time.ticket_id
group by 1, 2
